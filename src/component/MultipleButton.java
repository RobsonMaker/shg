...
public class MultipleButton extends JPanel {
    ...
    public ArrayList<Quarto> getItems() {
        return items;
    }

    public void setItems(ArrayList<Quarto> items) {
        this.items = items;
        initItem();
    }
    ...
    private ArrayList<Quarto> items = new ArrayList();
    ...
    private JLabel createLabe(Quarto data) {
        JLabel label = new JLabel("<html> <p><b>Número: </b></p>" + data.getNumero_quarto() + "<br /> <p><b>Preço: </b></p>" + data.getTipo().getPreco() + "</html>");
        label.setFont(label.getFont().deriveFont(Font.PLAIN, 14));
        label.setHorizontalAlignment(SwingConstants.CENTER);
        System.out.println(data.isStatus());
        if (data.isStatus().equals("Disponivel") || data.isStatus().equals("Disponível")) {

            label.setForeground(new Color(115, 236, 51));
        } else {
